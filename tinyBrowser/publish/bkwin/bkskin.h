...
class CBkSkinPngHorizExtend
	: public CBkSkinBase
{
	BKOBJ_DECLARE_CLASS_NAME(CBkSkinPngHorizExtend, "pnghorzex")

public:
	CBkSkinPngHorizExtend()
		: m_uResID(0)
		, m_lSubImageWidth(0)
		, m_lSkinParamLeft(0)
		, m_pImg(NULL)
	...
	virtual void Draw(Gdiplus::Graphics* dc, CRect rcDraw, DWORD dwState)
	{
		if (m_pImg)
		{
			SIZE size = {0, 0};
			...
			int iExtend = m_lSkinParamLeft * 2;

			if (m_lSubImageWidth * dwState >= m_pImg->GetWidth())
			{
				dwState = 0;
			}

			if (rcDraw.Width() < iExtend &&
				m_iDrawFullImg == 0)
			{
				int cx = rcDraw.Width() / 2;

				dc->DrawImage(m_pImg, Gdiplus::Rect(rcDraw.left, rcDraw.top, cx, size.cy), \
					m_lSubImageWidth * dwState, 0, cx, size.cy, Gdiplus::UnitPixel);


				dc->DrawImage(m_pImg, Gdiplus::Rect(rcDraw.left + cx, rcDraw.top, cx, size.cy), \
					m_lSubImageWidth * dwState + size.cx - cx, 0, cx, size.cy, Gdiplus::UnitPixel);
			}
			else
			{
				// 			LOG_WARN1(_T("CBkSkinPngHorizExtend::Draw--dwState:  %d\n"), dwState);
				// 			LOG_WARN4(_T("CBkSkinPngHorizExtend::Draw:  %d,%d,%d,%d\n"), m_lSubImageWidth * dwState, m_lSkinParamLeft, size.cx, size.cy - iExtend);
				dc->DrawImage(m_pImg, Gdiplus::Rect(rcDraw.left, rcDraw.top, m_lSkinParamLeft, size.cy), \
					m_lSubImageWidth * dwState, 0, m_lSkinParamLeft, size.cy, Gdiplus::UnitPixel);
				// 			LOG_WARN4(_T("Head::Draw:  %d,%d,%d,%d"), m_lSubImageWidth * dwState, 0, m_lSkinParamLeft, size.cy);

				dc->DrawImage(m_pImg, Gdiplus::Rect(rcDraw.left + m_lSkinParamLeft, rcDraw.top, rcDraw.Width() - iExtend, size.cy), \
					m_lSubImageWidth * dwState + m_lSkinParamLeft, 0, size.cx - iExtend, size.cy, Gdiplus::UnitPixel);

				// 			LOG_WARN4(_T("Body::Draw:  %d,%d,%d,%d"), m_lSubImageWidth * dwState + m_lSkinParamLeft, 0, size.cx - iExtend, size.cy);

				dc->DrawImage(m_pImg, Gdiplus::Rect(rcDraw.left + rcDraw.Width() - m_lSkinParamLeft, rcDraw.top, m_lSkinParamLeft, size.cy), \
					m_lSubImageWidth * dwState + size.cx - m_lSkinParamLeft, 0, m_lSkinParamLeft, size.cy, Gdiplus::UnitPixel);

				// 			LOG_WARN4(_T("Tail::Draw:  %d,%d,%d,%d"), m_lSubImageWidth * dwState + size.cx - m_lSkinParamLeft, 0, m_lSkinParamLeft, size.cy);
			}
	...
	virtual int GetSkinSubCount()
	{
		if ((0 != m_lSubImageWidth) &&
			(NULL != m_pImg))
		{
			return (m_pImg->GetWidth() + m_lSubImageWidth - 1) / m_lSubImageWidth;
		}
		return 1;
	}
...
protected:
	...
	BKWIN_DECLARE_ATTRIBUTES_BEGIN()
		...
		BKWIN_INT_ATTRIBUTE("subwidth", m_lSubImageWidth, TRUE)
		BKWIN_INT_ATTRIBUTE("left", m_lSkinParamLeft, TRUE)
		BKWIN_INT_ATTRIBUTE("drawfullimg", m_iDrawFullImg, TRUE)
	BKWIN_DECLARE_ATTRIBUTES_END()
};
...
//     }
    ...
    static CBkSkinBase* _CreateBkSkinByName(LPCSTR lpszName)
    {
		...
		pNewSkin = CBkSkinPngHorizExtend::CheckAndNew(lpszName);
        if (pNewSkin)
            return pNewSkin;
